...
#define CDataStructs_List_LogMessage(message) printf("%s\n", message)
    ...
    void CDataStructs_List_ShiftItemsLeft(CDataStructs_List* list, int32_t index);
    ...
    void CDataStructs_List_RemovePos(CDataStructs_List* list, uint32_t pos, void(*freeMethod)(void*))
    {
        ...
        {
            printf("CDataStructs Error: List Remove || The List does not have a item at %i, append one with \"Append\" or \"AppendDuplicate\" first!\n", pos);
            return;
        }
        if (freeMethod)
    ...
    void CDataStructs_List_Print(CDataStructs_List* list)
    {
        ...
        printf("CDataStructs_List: Item Count: %i, Item Memory Size: %i, List Total Size: %i\n", list->length,
